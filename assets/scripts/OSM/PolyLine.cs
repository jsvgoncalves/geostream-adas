...
public class PolyLine {
	...
	public GameObject Close(Transform parent)
	{
		...
		Loom.RunAsync(()=>{
			...
			for(int i = 0; i < this.nodes.Count - 2; i++) // here we create the vertexes
			{
				...
				if(i==0)
				{
					Vector3 p0 = new Vector3((float)(nodes[i].easthing - offsetX)+perpDirL.x,0.1f + nodes[i].height , (float)(nodes[i].northing - offsetY)+perpDirL.y);
					Vector3 p1 = new Vector3((float)(nodes[i].easthing - offsetX)+perpDirR.x,0.1f + nodes[i].height, (float)(nodes[i].northing - offsetY)+perpDirR.y);
					previousP2 = p0;
					previousP3 = p1;
                    if (dot < 0)
                    {
				...
				if(i== nodes.Count-3)
				{
					...
					Vector3 p4 = new Vector3((float)(nodes[i + 2].easthing - offsetX) + perpDirL.x, 0.1f + nodes[i+2].height, (float)(nodes[i + 2].northing - offsetY) + perpDirL.y);
					Vector3 p5 = new Vector3((float)(nodes[i + 2].easthing - offsetX) + perpDirR.x, 0.1f + nodes[i+2].height, (float)(nodes[i + 2].northing - offsetY) + perpDirR.y);
